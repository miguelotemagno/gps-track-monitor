...
function getCreateMarkerJS() {
     ...
     $_output .= sprintf('if(isArray(html)) { GEvent.addListener(marker, "%s", function() { var punto = buscar(marker.getPoint());actualmarker = gmarkers22[punto].getPoint(); if (ruta) {    setRoutePoints();  }marker.openInfoWindowTabsHtml(html);}); }',$this->window_trigger) . "\n";
            $_output .= sprintf('else { GEvent.addListener(marker, "%s", function() { var punto = buscar(marker.getPoint());actualmarker = gmarkers22[punto].getPoint();if (ruta) {    setRoutePoints();  }marker.openInfoWindowHtml(html); }); }',$this->window_trigger) . "\n";
