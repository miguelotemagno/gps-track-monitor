...
class GoogleMapAPI {
    ...
    */                                                                                                                           
    ...
    function getMapJS() {
        ...
        $_output .= 'var FECHA;';
        $_output .= 'var swg=new Array();var sw=0;var dir;var recorrido=new Array();var Direcc=new Array ();var tiemporecorrido=0;var delay=2000;var distancia;var gmarkers =[];var cont=0;var cont2=new Array();var htt=new Array ();var HORA2=new Array ();var lat=new Array();var lngg=new Array ();var lng=new Array ();var point=new Array ();var pat=new Array ();var mar=new Array ();var mod=new Array ();var fech=new Array ();var hora=new Array ();var vel=new Array ();var total=new Array();';
...
function distncia($origen, $destino, $pasa_por = ""){
	...
	if($pasa_por != ""){
		$_pasa_por = $pasa_por;
		if(is_array($pasa_por)){//si es un array
			$out = $_pasa_por = "";
			foreach($pasa_por as $value){
				$out .= "to:".$value."+";
				$_pasa_por .= $value.", ";
			}
			$_pasa_por = substr($_pasa_por, 0, -2);
			$pasa_por = $out;
		} else {
			$pasa_por = $pasa_por."+";
		}
		...
		$salida2["url"] = $url;
		$salida2["origen"] = $origen;
		$salida2["destino"] = $_destino;
		$salida2["pasa_por"] = $_pasa_por;
	} else {
		$url = "http://maps.google.es/maps?f=d&hl=es&saddr=".$origen."&daddr=".$destino;
		$salida2["url"] = $url;
		$salida2["origen"] = $origen;
	...
	$datos = str_replace("\u0026", "#2#", $query);
	$datos = str_replace("\u003e", "#1#", $datos);
	$datos = str_replace("\u003c", "#3#", $datos);
	$datos = str_replace("class=\\\"dditd noprint\\\"#1##3#div#1#", "#fin#", $datos);
	$datos = str_replace("class=\\\"noprint\\\"", "#fin1#", $datos);
	$datos = str_replace("Novedad!#3#/span#1# Arrastra la lí­nea de color azul para cambiar la ruta.#3#/div#1##3#div #fin#", "#fin2#", $datos);
	preg_match_all($exp_info, $datos, $info);
	if(!isset($info[1][0])){
		$exp_info = '!"#1##3#div #fin1##1##3#div#1#(.+)#2##160;km #2##8211; aprox. (.+)#3#!U';
		preg_match_all($exp_info, $datos, $info);
	}
	//
	$salida2["tiempo"] = $info[2][0];
	$tiempo = $info[2][0];
	//
	$dias[0] = @explode(" dí­as y ", $tiempo);
	$dias[1] = @explode(" dí­a y ", $tiempo);
	if($dias[0][0] == $tiempo){
		$tiempo = $dias[1][1];
		$salida2["dias"] = $dias[1][0];
		$dias = "";
	}
        echo $dias[0][0];
	if($dias[1] == $tiempo){
	...
	//
	$horas[0] = @explode(" horas", $tiempo);
	$horas[1] = @explode("h ", $tiempo);
	if($horas[0][0] != $tiempo){
		$tiempo = $horas[0][1];
		$salida2["horas"] = $horas[0][0];
		$horas = "";
	}
	if($horas[1][0] != $tiempo){
		$tiempo = $horas[1][1];
		$salida2["horas"] = $horas[1][0];
		$horas = "";
	}
	if($salida2["horas"] == ""){
		$tiempo = $info[2][0];
	...
	return $salida2;
        ...
        function quitar(){
           $eo="function quitarPubli(){";

                $eo.="var mapa = document.getElementById(\"map_canvas\");";
                //Dibujo de Google
                $eo.="var dibujo = mapa.childNodes[2]";
                $eo.="dibujo.innerHTML =\"\";";

                //Informacion de Google
                $eo.="var dibujo = mapa.childNodes[1];";
                $eo.="dibujo.innerHTML = \"\";";
...
$ss.="Concurrent.Thread.create(mostrar,iii,cont,tot,false);\n \n cont2[iii]++;";
$ss.="}\n }</script>";
    return $ss;
}
function despliega(){
           $ss.="\n\n<script>function mostrar(sss,cc,tt,pline){";
       $ss.=" \n var ss=sss; if (swg[ss]==1){for (iu=0;iu<tt;iu++){\n";
//for (iu=0;iu<tt;iu++){\n";
$ss.="if (pline) { if (cc>0){\n";
 $ss.="var lati=lat[ss][cc-1];var longg=lng[ss][cc-1];var lati2=lat[ss][cc];var long2=lng[ss][cc];\n";


        $ss.="var polyline = new GPolyline([
       ...
       $ss.="var marker= createMarker(point[ss][cc], 'Informacion','<div>'+pat[ss][cc]+mar[ss][cc]+mod[ss][cc]+hora[ss][cc]+fech[ss][cc]+vel[ss][cc]+Direcc[cc]+recorrido[ss][cc]+'<\/div>', cc,'');\n";
        $ss.="gmarkers.push(marker); \n if (cont>0) map.removeOverlay(gmarkers[cc-1]);\n  map.addOverlay(marker);\n";
        //$ss.= "map.setCenter(new GLatLng(lat[ss][cc],lng[ss][cc]), 13, G_NORMAL_MAP); \n ";
       ...
       // $ss.=" directionsPanel = document.getElementById(\"route\");";
      //$ss.="directions = new GDirections(map, directionsPanel);";
      //$ss.="directions.load(\"from: \");";
      $ss.="cc++; \n \n  Concurrent.Thread.sleep (delay);";
      $ss.="}}}</script>";
       return $ss;
}
function GetNewLoad(){
       ...
       $ss="<script>";
  		...
  		$ss.="function getnewload(fecha){\n";
               /* $ss.="for(var y=0;y<sw;y++){ for (var i=0;i<total[y];i++){\n";
                       $ss.="point[y][i]=\"\";\n";
                       $ss.="pat[y][i]=\"\";\n";
                ...
                $ss.="var op=document.getElementById('Radio_0').checked ; if (!op){ var op2=document.getElementById('Radio_1').checked ;}\n";
		    ...
		    $ss.="request.onreadystatechange = function () {\n";
				$ss.="if (request.readyState == 4) {\n";
					...
					$ss.="sw=0;var data=\"\";var ii=0;if (locations.length){";
                                                $ss.="cont2[sw]=0;recorrido[sw]=new Array();vel[sw]=new Array();hora[sw]=new Array();fech[sw]=new Array();mod[sw]=new Array();mar[sw]=new Array();lat[sw]=new Array();lng[sw]=new Array();point[sw]=new Array();pat[sw]=new Array();";
						$ss.="for (var i = 0; i < locations.length; i++) {\n"; // cycle thru locations
                                                $ss.="var dat=locations[i].getAttribute('id');if (sw==0){ data=locations[i].getAttribute('id');sw=1;}";
                                                $ss.=" if(data==dat){}else{cont2[sw]=0;ii=0;recorrido[sw]=new Array();vel[sw]=new Array();hora[sw]=new Array();fech[sw]=new Array();mod[sw]=new Array();mar[sw]=new Array();lat[sw]=new Array();lng[sw]=new Array();point[sw]=new Array();pat[sw]=new Array();data=locations[i].getAttribute('id');sw=sw+1; }";
                                                $ss.="rr=sw-1;lat[sw-1][ii]=(locations[i].getAttribute('lat'));";
                                                $ss.="total[sw-1]=ii+1;lng[sw-1][ii]=locations[i].getAttribute('lng');";
                                                $ss.="point[sw-1][ii]= new GLatLng(locations[i].getAttribute('lat'),locations[i].getAttribute('lng'));\n";
                                                $ss.="pat[sw-1][ii]='<b>Patente: </b>'+locations[i].getAttribute('patente')+'<br>';\n";
                                                $ss.="mar[sw-1][ii]=' <b>Marca: </b>'+locations[i].getAttribute('marca')+'<br>';\n";
                                                $ss.="mod[sw-1][ii]=' <b>Modelo </b>'+locations[i].getAttribute('modelo')+'<br>';\n";
                                                $ss.="fech[sw-1][ii]=' <b>Fecha: </b>'+locations[i].getAttribute('fecha')+'<br>';\n";
                                                $ss.="hora[sw-1][ii]=' <b>Hora: </b>'+locations[i].getAttribute('hora')+'<br>';\n";
                                                $ss.="vel[sw-1][ii]=' <b>Velocidad: </b>'+locations[i].getAttribute('velocidad')+' km/h<br>';\n";
                                                
                                                $ss.="recorrido[sw-1][ii]=' <b>Kilometros Recorridos: </b>'+locations[i].getAttribute('dist')+'<br>';\n";
                                                
                                                $ss.="ii++;tiemporecorrido=locations[i].getAttribute('dist');\n";
                                                /*$ss.=" var l=lat[i];var g=lng[i];latlng=l+\",\"+g;     if (latlng) {
                                                        geocoder.getLocations(latlng, function(addresses) {
                                                          if(addresses.Status.code != 200) {}
                                                          else {
                                                            address = addresses.Placemark[0];
                                                            htt[i] = address.AddressDetails.Country.CountryName;;
...
*/
                                                 ...
                                                 // $ss.="var marker= createMarker(point, 'Informacion','<div>'+pat+mar+mod+hora+fech+vel+'<\/div>', i,'');\n";
                                                 // $ss.="map.addOverlay(marker);\n";
                                              
						$ss.="}";
                                                $ss.="cont=0;dir=null;setTimeout(\"gbelmm();\", 2500);";
                                                $ss.="var d=direccion(point[cont]);";
                                                $ss.= "\n";
					$ss.="}";
                                        $ss.= "\n";
			...
			$ss.="}";
                        $ss.= "\n";
			$ss.="request.send(null);}else{alert(\"Ingrese Fecha!\");}";
	    $ss.="}";
  ...
  return $ss;
    ...
    function putCache($address, $lon, $lat) {
        if(!isset($this->dsn) || (strlen($address) == 0 || strlen($lon) == 0 || strlen($lat) == 0))
           return false;
        // PEAR DB
        require_once('file:///D|/GoogleMapAPI-2.5/GoogleMapAPI-2.5/DB.php');          
        $_db =& DB::connect($this->dsn);
        if (PEAR::isError($_db)) {
            die($_db->getMessage());
        }
