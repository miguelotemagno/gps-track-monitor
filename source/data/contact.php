...
if (empty($action)) {
	...
	<div class='contact-content'>
		<h1 class='contact-title'>Ingreso Usuarios</h1>
		<div class='contact-loading' style='display:none'></div>
		<div class='contact-message' style='display:none'></div>
		<form action='#' style='display:none'>
			<br><br><br><label for='contact-name'>User:</label>
	...
	if ($extra["form_subject"]) {
		$output .= "
			...
			<input type='password' id='contact-subject' class='contact-input' name='subject' value='' tabindex='1003' />";
	}

	
	$output .= "
			<label>&nbsp;</label>
			<button type='submit' class='contact-send contact-button' tabindex='1006'>Ingresar</button>
...
session_start();
  ...
  if (isset($_POST["name"]) && isset($_POST["subject"])){
     ...
     $sql="SELECT * FROM usuarios WHERE  usuario='".$_POST["name"]."' AND pass='".$_POST["subject"]."'";
     ...
     if (mysql_affected_rows()>0){
       ...
       echo "Bienvenido al Sistema GPS.";
        exit;

     }

     else{
          echo "No se puede ingresar, verifica el usuario y/o password";
                 }
