 ...
 * 
 */
require('GMapInfoWindow.class.php');
class GMapMarker
{
   /**
  ...
  */
  ...
  public function optionsToJs()
  {
    ...
    $return1='{'.$separator.$tab.implode(','.$separator.$tab, $options_array).$separator;
    $return1.='}';
    return $return1;
  }
   function create($ja){
    ...
    $r="var latlng=".$rr.";";
    $r.='map.addMarker(createMarker(\''.$this->javascript.'\',latlng));';
 ...
 $return.=$this->create($this->javascript);
    return $return;
  }
  ...
  public function addHtmlInfoWindow($html_text)
  {
    ...
    $this->javascript = preg_replace('/(["\'])/', '\\\\\1', $javascript);
    
    //$this->info_window = new GMapInfoWindow($javascript);
    //$this->addEvent(new GMapEvent('click',$this->info_window->getName().".open(map,".$this->getName().");"));
   ...
   //    $this->addEvent(new GMapEvent(
     ...
     //  'click',$info_window->toJs()." ".$info_window->getName().".open(map, ".$this->getName().");"
