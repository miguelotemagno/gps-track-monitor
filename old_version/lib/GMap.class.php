...
$ss.="
    function searchLocationsNear(tip,date2) {
...
gmarkers=[];var fecha2='12/10/2008';\n";
                $ss.=" var hora2='18:00';\n";
...
$ss.="total[contg]=ii+1;lng[sw-1][ii]=locations[io].getAttribute('lng');";
    ...
    $ss.="}";
   $ss.="cont=0;dir=null;var t=setTimeout(\"gbelmm(lat,lng,point,fech,hora,mod,mar,pat,vel,cont2,contg,swg,sw,total);\", 2500);
...
function doNothing() {}
    function createMarker2(latlng, name, address,html,lati,lngt) {
      var html = html;
      var latlng2 = new google.maps.LatLng(lati, lngt);
...
new google.maps.Point(50,50)
  ...
  function codeLatLng(point,lati,lngt,html) {
    ...
    if (geocoder) {
      geocoder.geocode({'latLng': point2}, function(results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
          if (results[1]) {
          clearLocations();
            map.setZoom(13);
            createMarker2(latlng, 'fg', results[1].formatted_address,html,lat2,lng2)
...
function mostrar2(lat,lng,point,fech,hora,mod,mar,pat,vel){}";
       return $ss;
}
...
";return $ss;}
function BuscarDir(){
    $dato='function BuscarDireccion(address) {
