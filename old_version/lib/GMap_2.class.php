...
$r.="\n";
  $r.='var marker = new google.maps.Marker({position: latlng, map: map});;';
  $r.="\n";
...
$ss.="function searchLocationsNear(tip,date) {
...
for(var y=0;y<contg;y++){ swg[y]=0;for (var i=0;i<total[y];i++){\n";
		    ...
		    $ss.="var urlstr='controlador/run.php?opt='+tip+'&date='+date+'&idv='+pate+'&iduser=0001';\n
...
function dir1prim(event){
            var request = {
              origin: event.latLng.toUrlValue(6),
              destination: event.latLng.toUrlValue(6)
...
function dir2prim(event){
    var origen=markersArray[markersArray.length - 1].getPosition();
 var request = {
              origin: origen,
              destination: event.latLng.toUrlValue(6)
