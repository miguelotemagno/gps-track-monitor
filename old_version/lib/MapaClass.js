...
function MapaJSClass(tip,date2w,Vehiculos){
     ...
     function BuscarMain() {
                        ...
                        downloadUrl(searchUrl, function(data) {
                                ...
                                if (!locations[0]){sinreg();clearLocations2();return null;}
                             ...
                             clearLocations2();
                                ...
                                rungps(date2,patenter,cantidad,2);if(statusalert){aler();}
...
function DataGPS(date,patentess,total,star){
                    try {
                                ...
                                var disV=new Array();
                                   ...
                                   Concurrent.Thread.create(jump,urlstr,latV,lonV,pointV,fechV,horaV,modV,marV,velV,disV,patentes);


                        }var star2=star+1;rungps(date,patentes,total,star2);if(statusalert){aler();}
                    } catch(e) {
					alert(e.name + " - " + e.message);
				}
                            }

function jump(urlstr,latV,lonV,pointV,fechV,horaV,modV,marV,velV,disV,patentes){
     downloadUrl(urlstr, function(data) {
                                            var xml = parseXml(data);
                                            var locations = xml.documentElement.getElementsByTagName("marker");
                                            if (locations.length==0){initd();clearLocations2();return null;}
                                           ...
                                           mostrar(latV,lonV,pointV,fechV,horaV,modV,marV,patentes,velV,disV,locations.length);
